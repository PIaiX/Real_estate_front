...
import {onInputHandler, onCheckboxHandler, onMultiCheckboxHandler} from './collectDataFromForm'
import useWindowDimensions from '../hooks/useWindowDimensions';

const CatalogOffcanvasFilters = ({filters, setFilters, foundCount, onResetFilters, onSumbitHandler, isShowCanvas, setIsShowCanvas, isShowMap, ...props}) => {
    const {width} = useWindowDimensions()
    const handleClose = () => setIsShowCanvas(false)
    const handleShow = () => setIsShowCanvas(true)

    return (
        <Offcanvas
            bsPrefix={isShowMap ? 'offcanvas offcanvas-when-shown-map' : null}
            placement='start'
            show={isShowCanvas}
            onHide={handleClose}
            data-bs-scroll={!isShowMap}
            data-bs-backdrop={!isShowMap}
            {...props}
        >
            <Offcanvas.Header>
                {
                    width < '991' && isShowCanvas
                        ? <button
                            type="button"
                            className="btn-close"
                            onClick={handleClose}
...
export default CatalogOffcanvasFilters;